    ...
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4) {
            if (this.status == 200) {
                ...
                if (objects.length === 0) {
                    document.getElementById("table").style.display = "none";
                    document.getElementById("notify").innerHTML = "Do not have task yet!";
                } else {
...
function deleteFolder() {
    ...
    if (selected) {
        Swal.fire({
            html: "You are about to delete <span class = 'thick'>" + text + "</span> folder!</p>",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#343A40',
            cancelButtonColor: '#d33',
            confirmButtonText: 'Uk',
